...
for audio in nonDem:
    ...
    file_location = audio.removeprefix(dir_path+'/InputAudioData/Non_DementiaFiles')
    file_type = file[1]
    wavFile = dir_path+'/non_dem_output/'+file[0]+"/"+file[0]+".wav"
    os.makedirs(dir_path+'/non_dem_output/'+file[0])
    f = open(dir_path+'/non_dem_output/'+file[0]+"/filedata.txt", "a")
    csvGen = open(dir_path+'/non_dem_output/'+file[0]+"/genData.csv", "a")
    ...
    if(file_type == ".wav"):
        if(verbose): print("Native Wave File:" + dir_path+'/non_dem_output/'+file[0]+"/"+file_name)
        shutil.copyfile(audio, dir_path+'/non_dem_output/'+file[0]+"/"+file_name)
    elif(file_type == ".mp3"):
        if(verbose): print("Converting Mp3 File:" + dir_path+'/non_dem_output/'+file[0]+"/"+file_name)
        sound = pydub.AudioSegment.from_mp3(audio)
        sound.export(wavFile, format="wav")
    elif(file_type == ".mp4"):
        if(verbose): print("Converting Mp4 File:" + dir_path+'/non_dem_output/'+file[0]+"/"+file_name)
        video = moviepy.editor.VideoFileClip(audio)
        #Extract the Audio
        audio = video.audio
        #Export the Audio
        audio.write_audiofile(wavFile)
    elif(file_type == ".cha"):
        if(verbose): print("Converting .cha File:" + dir_path+'/non_dem_output/'+file[0]+"/"+file_name)
